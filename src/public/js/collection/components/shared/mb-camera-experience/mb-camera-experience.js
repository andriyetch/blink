...
export class MbCameraExperience {
  ...
  render() {
    return (h(Host, { class: classNames({ 'no-overlay': !this.showOverlay }) },
      ...
      h("mb-camera-toolbar", { "show-close": this.apiState !== "error", "camera-flipped": this.cameraFlipped, onCloseEvent: () => this.handleStop(), onFlipEvent: () => this.flipCamera(), onChangeCameraDevice: (ev) => this.handleChangeCameraDevice(ev.detail), ref: el => this.cameraToolbar = el })));
  }
