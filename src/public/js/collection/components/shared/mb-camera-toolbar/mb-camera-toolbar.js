...
export class MbCameraToolbar {
  ...
  render() {
    ...
    if (this.enableCameraFlip) {
      flipButton = (h("a", { href: "javascript:void(0)", class: this.cameraFlipped ? 'toolbar-button flip-button flipped' : 'toolbar-button  flip-button', onClick: (ev) => this.handleFlip(ev) },
    ...
    let closeButton = '';
    if (this.showClose) {
      closeButton = (h("a", { href: "javascript:void(0)", class: "toolbar-button close-button", onClick: (ev) => this.handleClose(ev) },
    ...
    return (h(Host, null,
      h("header", null,
        flipButton,
        h("section", { class: "camera-selection-wrapper" },
          h("mb-camera-selection", { onChangeCameraDevice: (ev) => this.handleChangeCameraDevice(ev.detail), class: classNames({ visible: this.isDesktop }), ref: el => this.cameraSelection = el })),
        closeButton)));
  }
